...
from DYSAS.Vessel import GunnerusPID
...
if __name__ == "__main__":
    ...
    # Simulator class
    sim = Simulator(GunnerusPID(), dt=dt, T=T)
    
    # Controller
    sim.vessel.controller.kp = 8e4
    sim.vessel.controller.ki = 1e5
    sim.vessel.controller.T = 1e-1
    sim.vessel.controller.r = kn2ms(5)

    # Initial conditions
    sim.vessel.x[2] = deg2rad(0)    # Vessel is headed 30 degrees (clockwise)
    sim.vessel.x[3] = kn2ms(5)      # Vessel is stopped
